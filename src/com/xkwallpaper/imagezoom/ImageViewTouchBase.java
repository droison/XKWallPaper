...
import com.xkwallpaper.imagezoom.easing.Cubic;
import com.xkwallpaper.imagezoom.easing.Easing;
import com.xkwallpaper.imagezoom.graphics.FastBitmapDrawable;
import com.xkwallpaper.imagezoom.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
	...
	protected void onLayout( boolean changed, int left, int top, int right, int bottom ) {
		...
		if( changed ) {
			...
			// update center point ��ʾ������λ��
			mCenter.x = mThisWidth / 2f;
			mCenter.y = 0;//mThisHeight / 2f;
		}
	...
	protected void _setImageDrawable( final Drawable drawable, final Matrix initial_matrix, float min_zoom, float max_zoom ) {
		...
		if ( min_zoom != ZOOM_INVALID && max_zoom != ZOOM_INVALID ) {
			...
			Log.i( LOG_TAG, "mMaxZoom"+mMaxZoom );
	...
	public void zoomTo( float scale, float centerX, float centerY, final float durationMs ) {
		if ( scale > getMaxScale() ) scale = getMaxScale();
